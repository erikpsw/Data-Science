...
class DisjunctiveGraph:
    ...
    def __init__(self, jobs: List[Job]):
        ...
        # 添加虚拟开始和结束节点
        self.start_node = Operation(0, 0, 0, 0)  # 虚拟开始节点
        self.start_node.id = "START"
        self.end_node = Operation(-1, -1, -1, 0)  # 虚拟结束节点
    ...
    def _build_graph(self):
        ...
        # 添加虚拟节点到操作列表
        self.operations.insert(0, self.start_node)
        self.operations.append(self.end_node)
            
        # 按机器分组工序（不包括虚拟节点）
        for op in self.operations:
            if op.id not in ["START", "END"]:
    ...
    def _build_virtual_arcs(self):
        """构建虚拟弧 - 连接START到所有第一道工序，所有最后工序到END"""
        # START到每个工件的第一道工序
        for job in self.jobs:
            if job.operations:
                first_op = job.operations[0]
                self.conjunctive_arcs.append((self.start_node, first_op, 0))
        
        # 每个工件的最后工序到END
        for job in self.jobs:
            if job.operations:
                last_op = job.operations[-1]
                self.conjunctive_arcs.append((last_op, self.end_node, last_op.processing_time))
    ...
    def is_acyclic(self, machine_orders: Dict[int, List[Operation]], visualize: bool = False) -> bool:
        """
        检查给定机器顺序是否形成无环图
        
        Args:
            machine_orders: 每台机器上工序的加工顺序
            visualize: 是否在检测前可视化图结构
    ...
    def sample_valid_machine_order(self, max_attempts: int = 1000) -> List[Operation]:
        ...
        machine_orders = {}
        # print(f"无环: {self.is_acyclic(machine_orders)}")
...
class ScheduleSolver:
    ...
    def calculate_makespan(self, machine_orders: Dict[int, List[Operation]], visualize: bool = False) -> Tuple[int, Dict[str, int]]:
        """
        计算给定机器顺序下的最大完工时间（使用拓扑排序计算最长路径）
        ...
        Returns:
            (makespan, node_distances): 最大完工时间和各节点距离
        """
        # 构建有向图
        graph, in_degree = self.graph._build_directed_graph(machine_orders)
        
        # 执行拓扑排序并计算最长距离
        is_acyclic_result, distances = self.graph._topological_sort(graph, in_degree, compute_distances=True)
        
        # 检查是否为无环图，有环直接返回无穷大
        if not is_acyclic_result:
            if visualize:
                print("图包含环，无法计算makespan")
            return float('inf'), {}
        ...
        # 返回到结束节点的最长距离作为makespan
        makespan = distances[self.graph.end_node.id] if distances[self.graph.end_node.id] != float('-inf') else float('inf')
    ...
    def random_search(self, max_iterations: int = 1000, max_attempts=2000) -> Tuple[int, Dict[int, List[Operation]]]:
        ...
        for iteration in range(max_iterations):
            ...
            #     pass
            # 计算makespan
            makespan, _ = self.calculate_makespan(machine_orders, visualize=False)
            # print(makespan)
...
if __name__ == "__main__":
    ...
    main(choice=2, max_iterations=1000, max_attempts=3000000)
