...
class DisjunctiveGraph:
    ...
    def sample_valid_machine_order(self, max_attempts: int = 1000) -> Dict[int, List[Operation]]:
        ...
        for attempt in range(max_attempts):
            ...
            for machine in machines:
                ...
                # 增量检查是否有环（提前终止）
                if not self._is_partial_acyclic(machine_orders):
                    valid = False
                    break
    ...
    def _is_partial_acyclic(self, partial_orders: Dict[int, List[Operation]]) -> bool:
        """
        快速检查部分机器顺序是否可能导致环 - 用于提前终止
        
        Args:
            partial_orders: 部分机器顺序
            
        Returns:
            bool: True表示当前状态不会导致环
        """
        # 构建当前的有向图
        ...
        # 添加工艺路径约束边
        for from_op, to_op, _ in self.conjunctive_arcs:
            graph[from_op.id].add(to_op.id)
        
        # 添加已确定的机器顺序约束边
        for machine, ops in partial_orders.items():
            for i in range(len(ops) - 1):
                from_op = ops[i]
                to_op = ops[i + 1]
                graph[from_op.id].add(to_op.id)
        ...
        # 使用DFS检测环（针对当前涉及的节点）
        involved_nodes = set()
        for ops in partial_orders.values():
            for op in ops:
                involved_nodes.add(op.id)
        
        # 添加工艺路径中相关的节点
        for from_op, to_op, _ in self.conjunctive_arcs:
            if from_op.id in involved_nodes or to_op.id in involved_nodes:
                involved_nodes.add(from_op.id)
                involved_nodes.add(to_op.id)
        
        return self._dfs_no_cycle(graph, involved_nodes)
    
    def _dfs_no_cycle(self, graph: Dict[str, set], nodes: set) -> bool:
        """
        使用DFS检测指定节点集合中是否存在环
        ...
        def dfs(node):
            if colors[node] == GRAY:  # 发现后向边，存在环
                return False
            if colors[node] == BLACK:  # 已访问完成
                return True
    ...
    def sample_multiple_valid_orders(self, count: int = 10, max_attempts: int = 1000) -> List[Dict[int, List[Operation]]]:
        """
        批量采样多个有效的机器工序顺序
        ...
        Returns:
            List[Dict[int, List[Operation]]]: 有效的机器工序顺序列表
        """
        valid_orders = []
        
        for i in range(count):
            order = self.sample_valid_machine_order(max_attempts)
            if order:
                valid_orders.append(order)
            else:
                print(f"警告: 第 {i+1} 次采样失败")
                break
...
if __name__ == "__main__":
    ...
    main(choice=1, max_iterations=1000, max_attempts=3000000)
